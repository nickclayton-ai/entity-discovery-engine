...
def detect_low_speed_events(
    ...
    work = df.copy()
    work["is_low_speed"] = work["sog"] < low_speed_knots
    work["time_diff_min"] = work["timestamp"].diff().dt.total_seconds().div(60)
    ...
    for _, row in work.iterrows():
        if row["is_low_speed"]:
            if not current_event_points:
                ...
                prev_time = current_event_points[-1]["timestamp"]
                gap_min = (row["timestamp"] - prev_time).total_seconds() / 60
            ...
            summaries.append(
                {
                    ...
                    "mean_speed_knots": round(event_df["sog"].mean(), 2),
                    "min_speed_knots": round(event_df["sog"].min(), 2),
                    "max_speed_knots": round(event_df["sog"].max(), 2),
